...
import { ACTIVE_EVENTS, Event, getFeaturedEvent } from "@/lib/data";
...
export function EventProvider({
  ...
  const initialIndex = useMemo(() => {
    if (initialSlug) {
      const bySlug = ACTIVE_EVENTS.findIndex((e) => e.slug === initialSlug);
      if (bySlug !== -1) return bySlug;
    }
    const featured = getFeaturedEvent();
    const featuredIndex = ACTIVE_EVENTS.findIndex((e) => e.id === featured.id);
    return featuredIndex === -1 ? 0 : featuredIndex;
  }, [initialSlug]);

  const [activeEventIndex, setActiveEventIndex] = useState(initialIndex);

  const activeEvent = ACTIVE_EVENTS[activeEventIndex];

  const setActiveEvent = (event: Event) => {
    const index = ACTIVE_EVENTS.findIndex((e) => e.id === event.id);
    if (index !== -1) {
  ...
  const setActiveEventBySlug = (slug: string) => {
    const index = ACTIVE_EVENTS.findIndex((e) => e.slug === slug);
    if (index !== -1) {
      setActiveEventIndex(index);
    }
  };
